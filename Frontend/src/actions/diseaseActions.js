...
export const listDiseases = (params) => async (dispatch) => {
    ...
    let search = '';
    let page = 0;
    let url = `/api/whatgarbage/`
    if (params){
        if (params.crop){
            crop = params.crop;
            url = `/api/whatgarbage?crop=${crop}&search=${search}`
        }
        if (params.search){
        ...
        if (params.page){
            page = params.page;
            url = `/api/whatgarbage?crop=${crop}&search=${search}&page=${page}`
        }
    }
    try {
        dispatch({
...
export const listDiseaseDetails = (crop, disease) => async (dispatch) => {
    try {
        ...
        let url = `/api/whatgarbage/${crop}/${disease}`;
        const { data } = await axios.get(url);
...
export const createDisease = (formData) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.post(`/api/whatgarbage/`, formData, config)
